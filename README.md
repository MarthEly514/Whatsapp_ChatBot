...
- Python 3.8+
- Compte [Twilio](https://www.twilio.com/) avec WhatsApp activé
- Clé API [Hugging Face](https://huggingface.co/)
...
3. Obtenez votre essai gratuit ($15 de crédit)
...
3. Notez le numéro WhatsApp Twilio (ex: `whatsapp:+14155238886`)
...
1. **Envoyer un message** au numéro Twilio sandbox
2. Le bot répond automatiquement avec l'IA
...
**Solutions:**
1. Vérifiez que ngrok est lancé
2. Vérifiez l'URL dans Twilio (doit être HTTPS)
3. Vérifiez les logs: `tail -f logs/whatsapp_bot.log`

### Problème: Modèle Hugging Face lent/timeout

**Solutions:**
1. Le modèle se charge (cold start), attendez 20-30s
2. Changez pour un modèle plus petit (`flan-t5-large`)
3. Augmentez le timeout dans `huggingface_service.py`

### Problème: Twilio Auth Error

**Solutions:**
1. Vérifiez ACCOUNT_SID et AUTH_TOKEN
2. Assurez-vous que le compte n'est pas suspendu
3. Régénérez l'Auth Token si nécessaire

### Problème: Messages WhatsApp non envoyés

**Solutions:**
1. Vérifiez que le destinataire a rejoint le sandbox
2. Vérifiez le format du numéro (`whatsapp:+33...`)
3. Vérifiez le crédit Twilio
...
Pour toute question:
1. Vérifiez les logs: `logs/whatsapp_bot.log`
